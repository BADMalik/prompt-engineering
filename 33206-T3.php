...
$testFramework->run('Queue size does not exceed QUEUE_LIMIT', function (SimpleTestFramework $test) {
    ...
    startThreadPool();
    $output = ob_get_clean();
    $test->assertEquals("Error: Request queue exceeded limit of " . QUEUE_LIMIT, strpos($output, "Error: Request queue exceeded limit of " . QUEUE_LIMIT) !== false);
});
...
$testFramework->run('Number of threads does not exceed MAX_THREADS', function (SimpleTestFramework $test) {
    ...
    startThreadPool();
    $output = ob_get_clean();
    $test->assertEquals("Starting " . MAX_THREADS . " threads to handle " . (MAX_THREADS * MAX_TASKS_PER_THREAD + 1) . " requests...", strpos($output, "Starting " . MAX_THREADS . " threads to handle " . (MAX_THREADS * MAX_TASKS_PER_THREAD + 1) . " requests...") !== false);
});
...
$testFramework->run('Tasks assigned to threads do not exceed MAX_TASKS_PER_THREAD', function (SimpleTestFramework $test) {
    ...
    startThreadPool();
    $output = ob_get_clean();
    $test->assertEquals("Starting 1 threads to handle " . (MAX_TASKS_PER_THREAD + 1) . " requests...", strpos($output, "Starting 1 threads to handle " . (MAX_TASKS_PER_THREAD + 1) . " requests...") !== false);
});
...
$testFramework->run('System handles zero requests gracefully', function (SimpleTestFramework $test) {
    ...
    startThreadPool();
    $output = ob_get_clean();
    $test->assertEquals("Starting 0 threads to handle 0 requests...", strpos($output, "Starting 0 threads to handle 0 requests...") !== false);
});
...
$testFramework->run('Performance measurement logging works without errors', function (SimpleTestFramework $test) {
    ...
    $test->assertEquals($expectedOutput, strpos($output, $expectedOutput) !== false);
});
...
$testFramework->run('Graceful shutdown logs the shutdown status and waits for active threads to finish', function (SimpleTestFramework $test) {
    ...
    startThreadPool();
    $output = ob_get_clean();
    $test->assertEquals("Shutdown initiated. Waiting for child processes to finish...", strpos($output, "Shutdown initiated. Waiting for child processes to finish...") !== false);
});
...
$testFramework->run('Tasks are distributed evenly across threads', function (SimpleTestFramework $test) {
    ...
    startThreadPool();
    $output = ob_get_clean();
    $test->assertEquals("Starting " . MAX_THREADS . " threads to handle " . (MAX_THREADS * MAX_TASKS_PER_THREAD) . " requests...", strpos($output, "Starting " . MAX_THREADS . " threads to handle " . (MAX_THREADS * MAX_TASKS_PER_THREAD) . " requests...") !== false);
});
...
$testFramework->run('Threads terminate properly after completing tasks', function (SimpleTestFramework $test) {
    ...
    startThreadPool();
    $output = ob_get_clean();
    $test->assertEquals("All threads completed.", strpos($output, "All threads completed.") !== false);
});
...
$testFramework->run('Log messages are formatted with timestamps', function (SimpleTestFramework $test) {
    ...
    $output = ob_get_clean();
    $test->assertEquals("[" . date('H:i:s') . "] Test log message", strpos($output, "[" . date('H:i:s') . "] Test log message") !== false);
});
