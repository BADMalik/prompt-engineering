...
$testFramework->run('Test encryption of long data', function($test) {
    $keys = generateKeyPair();
    $data = str_repeat("A", 256); // Data longer than RSA limit
    $encrypted = encryptData($data, $keys['public']);
    $decrypted = decryptData($encrypted, $keys['private']);
    $test->assertEqual($data, $decrypted);
});
...
// Verify PEM format
$testFramework->run('Verify PEM format for private key', function($test) {
    $keys = generateKeyPair();
    $test->assertStringContains("-----BEGIN RSA PRIVATE KEY-----", $keys['private']);
    $test->assertStringContains("-----END RSA PRIVATE KEY-----", $keys['private']);
});

$testFramework->run('Verify PEM format for public key', function($test) {
    $keys = generateKeyPair();
    $test->assertStringContains("-----BEGIN PUBLIC KEY-----", $keys['public']);
    $test->assertStringContains("-----END PUBLIC KEY-----", $keys['public']);
});

// Ensure HMACs detect message changes
$testFramework->run('Ensure HMACs detect message changes', function($test) {
    $data = "test data";
    $secretKey = "secret_shared_key";
...
// Verify signatures fail with altered messages or incorrect keys
$testFramework->run('Verify signatures fail with altered messages', function($test) {
    $keys = generateKeyPair();
    $data = "test data";
...
$testFramework->run('Verify signatures fail with incorrect keys', function($test) {
    $keysA = generateKeyPair();
    $keysB = generateKeyPair();
    $data = "test data";
...
// Verify uniqueness and proper length of generated nonces
$testFramework->run('Verify uniqueness and proper length of generated nonces', function($test) {
    $nonce1 = generateNonce();
    $nonce2 = generateNonce();
    $test->assertNotEqual($nonce1, $nonce2);
    $test->assertEqual(32, strlen($nonce1)); // 16 bytes hex-encoded
});

// Handle storing/retrieving keys to/from files
$testFramework->run('Handle storing/retrieving keys to/from files', function($test) {
    $keys = generateKeyPair();
    $filename = storePublicKey("TestSystem", $keys['public']);
    $retrievedKey = retrievePublicKey("TestSystem");
    $test->assertEqual($keys['public'], $retrievedKey);
    unlink($filename); // Clean up
});

// Test complete secure messaging workflow end-to-end
$testFramework->run('Test complete secure messaging workflow', function($test) {
    ...
    $test->assertEqual($dataWithNonce, $decrypted);
    $test->assertTrue(hash_equals($hmac, createHMAC($decrypted, $secretKey)));

    $signature = signData($data, $keysA['private']);
    $test->assertTrue(verifySignature($data, $signature, $keysA['public']));
});

// Test behavior with invalid inputs and non-existent resources
$testFramework->run('Test behavior with invalid inputs', function($test) {
    $test->assertFalse(verifySignature("data", "invalid_signature", "invalid_public_key"));
    $test->assertFalse(decryptData("invalid_encrypted_data", "invalid_private_key"));
});

$testFramework->run('Test non-existent key files', function($test) {
    $test->assertNull(retrievePublicKey("NonExistentSystem"));
});
