...
$testFramework->run('Log file resets after reaching max size', function ($test) {
    ...
    $logContent = $profiler->getLogContent();
    $test->assertTrue(filesize($profiler->getLogFile()) <= $profiler->getMaxLogSize(), "Log file should reset after reaching max size");
});
...
$testFramework->run('Log file size is monitored correctly', function ($test) {
    ...
    $test->assertTrue(filesize($profiler->getLogFile()) <= $profiler->getMaxLogSize(), "Log file size should be monitored correctly");
});
...
$testFramework->run('Log doesn\'t exceed limit even with many entries', function ($test) {
    ...
    $test->assertTrue(filesize($profiler->getLogFile()) <= $profiler->getMaxLogSize(), "Log should not exceed limit even with many entries");
});
...
$testFramework->run('Log file is truncated properly after reset', function ($test) {
    ...
    $logContent = $profiler->getLogContent();
    $test->assertTrue(filesize($profiler->getLogFile()) <= $profiler->getMaxLogSize(), "Log file should be truncated properly after reset");
});
