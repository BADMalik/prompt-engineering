...
$tester->run('Valid Payment Charge', function ($test) {
    $gateway = new StripePaymentGateway(false);
    $response = $gateway->charge(100.0, 'USD', 'tok_valid', 'client@example.com');

    $test->assertTrue($response->success, "Payment should succeed");
    $test->assertNotNull($response->transactionId, "Transaction ID should be set on success");
    $test->assertStringContains('Payment success', $response->message, "Message should indicate success");
});